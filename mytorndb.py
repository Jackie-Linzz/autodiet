...
class Connection(object):
    ...
    def insert_dict(self, tablename, rowdict, replace=False):
        ...
        if len(rowdict) > len(keys):
            ...
            logging.error('insert %s:', tablename)
    ...
    def insert_many_dict(self, table, rows, replace=False):
        cursor = self._cursor()
        cursor.execute('describe %s' % table)
        keys = set(i[0] for i in cursor.fetchall())
        keys = keys.intersection(rows[0])
        if len(keys) < len(rows[0]):
            unknown_keys = set(rows[0]) - keys
            logging.error('skipping keys: %s', ', '.join(unknown_keys))
        ...
        if replace:
            sql = "REPLACE INTO %s (%s) VALUES (%s)" % (
                table, columns, values_template)
        else:
            sql = "INSERT INTO %s (%s) VALUES (%s)" % (
                table, columns, values_template)

        values = [tuple(i[key] for key in keys) for i in rows]
